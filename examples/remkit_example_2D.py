...
========================
This is an example of the RemkitRun Connector class, displaying 2D metrics which vary across x through time.
...
To run this example on your own system with ReMKiT installed:
    ...
    - Run the example script: poetry run python examples/remkit_example_2D.py
...
with RemkitRun() as run:
    ...
    run.init(
        name="remkit-gaussian-advection-%s" % str(uuid.uuid4()),
    ...
    run.update_metadata({"example_name": "RMK_advection_test"})
    ...
    run.launch(
        ...
        config_path = pathlib.Path(__file__).parents[1].joinpath("tests", "example_data", "RMK_advection_test", "config.json"),
        # You can optionally use the connector to define where to store results, and which variables to track
        results_dir_path = str(pathlib.Path(__file__).parent.joinpath("2D_example_results")),
        clean_results_dir = True
    )
    
    # Once the simulation is complete, you can upload any final items to the Simvue run before it closes
    num_output_files = sum(1 for f in pathlib.Path(__file__).parent.joinpath("2D_example_results").iterdir() if f.is_file())
